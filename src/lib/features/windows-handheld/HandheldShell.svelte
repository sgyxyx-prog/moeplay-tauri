<script lang="ts">
  ...
  const tabs: { id: HandheldTab; label: string }[] = [{ id: "continue", label: "继续" }, { id: "library", label: "内容库" }, { id: "discover", label: "发现" }, { id: "mine", label: "我的" }];
  ...
  let panel = $state<"content" | "quick" | "search" | null>(null);
  ...
  const featured = $derived(panelItem ? handheldContentStore.library.find(item => item.id === panelItem?.id) ?? panelItem : selected);
  ...
  function showPanel(next: "content" | "quick" | "search") {
    if (panel) dismissPanel();
    list?.cancelFocus();
    panelItem = next === "content" ? selected : null;
  ...
  function move(direction: "up" | "down" | "left" | "right") {
    ...
    if (!panel && browse && (tab === "continue" || tab === "library") && active?.closest("[data-virtual-item-id]") && (direction === "down" || direction === "up")) {
  ...
  function activate() {
    const active = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    if (!panel && browse && active?.closest("[data-virtual-item-id]") && (tab === "continue" || tab === "library")) { void runPrimary(); return; }
  ...
  onMount(() => {
    ...
    shellScope = attachGamepad({ ...directions, launch: activate, back, favorite: () => showPanel("content"), activate: () => showPanel("search"), pageLeft: () => cycleTab(-1), pageRight: () => cycleTab(1), categoryLeft: () => cycleKind(-1), categoryRight: () => cycleKind(1), start: () => showPanel("quick") }, { id: "windows-handheld-shell", priority: 40, enabled: true });
...
<div bind:this={root} class="handheld-shell" class:compact={metrics.compact} class:light={displayProfile.profile.lightEffects} data-testid="windows-handheld-shell" style={`--hh-body:${metrics.body}px;--hh-aux:${metrics.auxiliary}px;--hh-target:${metrics.target}px;--hh-gap:${metrics.gap}px;--hh-keyboard:${inset}px`}>
  <header class="header">
  ...
  <main class="body" class:inner={!browse}>
    ...
    {:else}
      <div class="section-heading"><div><p>{tab === "continue" ? "PICK UP WHERE YOU LEFT OFF" : tab === "library" ? "YOUR COLLECTION" : "FIND YOUR NEXT STORY"}</p><h1>{tab === "continue" ? "接着上次" : tab === "library" ? "内容库" : "发现"}</h1></div><span>{tab === "discover" ? snapshot.query || "按名称搜索作品" : `${rows.length} 项内容`}</span></div>
      <div class="filters">
      ...
      {:else if rows.length}
        <div class="collection" class:dense={displayProfile.profile.density === "compact"}>
          ...
          {#if selected}<aside class="feature" aria-label="当前作品"><div class="hero">{#if selected.hero?.src || selected.cover?.src}<img src={selected.hero?.src || selected.cover?.src} alt="" loading="lazy" />{:else}<span>{labels[selected.kind]}</span>{/if}</div><div class="feature-copy"><small>{labels[selected.kind]} · {selected.subtitle || "随时继续"}</small><h2>{selected.title}</h2><p>{selected.progressLabel}</p>{#if selected.progress !== null}<progress value={selected.progress} max="1" aria-label="内容进度"></progress>{/if}<div class="feature-actions"><button class="primary" disabled={!primary?.enabled} onclick={runPrimary}>{selected.primaryLabel}</button><button onclick={() => showPanel("content")}>更多</button></div></div></aside>{/if}
        </div>
      {:else}
        <div class="empty"><span class="empty-symbol" aria-hidden="true">◈</span><h2>{tab === "continue" ? "从喜欢的内容开始" : "这里还没有内容"}</h2><p>{snapshot.filter === "local" ? "仅展示已确认在本机可用的内容。" : "游玩、观看或阅读后，可以从这里接着继续。"}</p><div><button class="primary" onclick={() => switchTab("library")}>打开内容库</button><button onclick={() => void gameStore.importGame()}>导入游戏</button><button onclick={() => showPanel("search")}>搜索内容</button></div></div>
      {/if}
    {/if}
  </main>
  <footer class="hints"><span><kbd>{glyph("launch")}</kbd>{panel ? "确认" : browse && selected && tab !== "mine" && tab !== "discover" ? selected.primaryLabel : "确认"}</span><span><kbd>{glyph("back")}</kbd>返回</span><span><kbd>{glyph("favorite")}</kbd>更多</span><span><kbd>{glyph("activate")}</kbd>搜索</span><span class="shoulder-hint"><kbd>{glyph("pageLeft")} / {glyph("pageRight")}</kbd>栏目</span><button onclick={() => showPanel("quick")}>{taskActiveCount ? `${taskActiveCount} 项任务` : "快捷设置"}</button></footer>
  <Drawer open={panel !== null} title={panel === "search" ? "搜索内容" : panel === "quick" ? "快捷面板" : featured?.title || "内容面板"} onClose={dismissPanel} bind:ref={panelRoot} class="handheld-drawer" initialFocus={panel === "search" ? "input" : "auto"} returnFocus={false}>
    {#if panel === "search"}
